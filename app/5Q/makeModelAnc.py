...
for i, row in enumerate(ws.iter_rows(values_only=True)):
    i = i+1
    if(i>=90534):
        if row[1] is not None and row[2] is not None and row[3] is not None and row[5] is not None and row[6] is not None and row[7] is not None:
            X.append([row[1] + row[2], row[3], row[5], row[7]])
            Y.append(row[6])
...
joblib.dump(model, script_path / "model" /"5Q_fullAnc.pkl")
joblib.dump(scaler, script_path / "model" /"5Q_fullAnc_scaler.pkl")