...
for i, row in enumerate(ws.iter_rows(values_only=True)):
    ...
    if(i>=17798):
        if row[1] is not None and row[2] is not None and row[3] is not None and row[4] is not None and row[6] is not None:
            X.append([row[1] + row[2], row[3], row[4], row[8], row[9]])
            Y.append(row[6])
...
joblib.dump(model, script_path / "model" /"4Q_1602_AncTot.pkl")
joblib.dump(scaler, script_path / "model" /"4Q_1602_AncTot_scaler.pkl")