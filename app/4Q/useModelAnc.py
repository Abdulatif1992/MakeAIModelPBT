...
model = joblib.load(script_path / "model" / "4Q_1602_Anc.pkl")
scaler = joblib.load(script_path / "model" / "4Q_1602_Anc_scaler.pkl")
...
for i, row in enumerate(ws.iter_rows(values_only=True)):
    ...
    if(i>=5):
        if row[1] is not None and row[2] is not None and row[3] is not None and row[5] is not None and row[6] is not None:
            ...
            total = row[4]
            anchorAvg = row[8]
            new_data = np.array([[s1s2, s3, total, anchorAvg]])
